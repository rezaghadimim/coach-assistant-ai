# AI-Execution Roadmap — ARCHIVED (COMPLETE)

> **STOP.** This tree is historical. Do not follow the execution protocol below for new work.
> Living contract: root `CLAUDE.md`. Living technical docs: `docs/MODULE_MAP.md`, `CONVENTIONS.md`, `CONTRACTS.md`, etc.
> Redirect stub: `docs/roadmap/README.md`.

**Objective (historical):** make every future implementation session on this repository reliable when executed by a small language model (7B–8B, limited context, no memory of prior sessions).
...
## 1. Execution protocol (historical — do not follow)

~~Former binding rules for T-xxx sessions.~~ **Superseded 2026-07-14.** All indexed tasks are `DONE`. Agents must not pick tasks from this archive. Remaining sections below are historical record only.
